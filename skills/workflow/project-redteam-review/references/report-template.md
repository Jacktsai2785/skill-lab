...
```bash
python3 scripts/render_html.py <專案>/REDTEAM-REVIEW.json --consume
```
...
## 套用關卡
HTML 交付後問使用者：要套用「排序行動項」中標 ✅（高信心低風險）的哪幾條？同意才動 code，逐項套用並回報 diff 摘要。標 ❌ 的不自動動手。套用後把 JSON 重建並以 `meta.applied=true` 重渲染（橫幅變「已套用」）。
