...
**在同一則訊息裡並行派出三個 Agent 呼叫**。建議 `subagent_type: general-purpose`。
...
規則：
- 只能基於事實基準與初衷提問；找不到根據（code / 文檔 / 流程）的猜測不要寫。
...
## 給主流程的彙整提示
- 收齊三份後，按 `location` + `problem` 主題歸併去重；記住「**幾隊點到**」——三隊都點到 ＝ 高信心，只一隊點到 ＝ 觀點差異。
